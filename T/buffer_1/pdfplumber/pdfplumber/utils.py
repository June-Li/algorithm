...
def image_to_edges(image):
    ...
    if image['width']>=3 and image['height']>=3:
        return []
    top, bottom, left, right = [ dict(image) for x in range(4) ]
    center_v=left
    center_h=bottom
    center_v.update({"object_type": "image_edge",
        "height": image['height'],
        "width": decimalize(0),
        "x0": decimalize(0.5)*(image["x0"]+image["x1"]),
        "x1": decimalize(0.5)*(image["x0"]+image["x1"]),
        "orientation": "v"})
    center_h.update({"object_type": "image_edge",
        "height": decimalize(0),
        "y0": decimalize(0.5)*(image["y0"]+image["y1"]),
        "y1": decimalize(0.5)*(image["y0"]+image["y1"]),
        "orientation": "h"})    
    # print('111',top)
