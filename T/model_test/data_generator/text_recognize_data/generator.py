...
def sto_choice_from_info_str(quantity=10):
    ...
    biaodian = list(',.{}[]:;()!，。【】《》：；！')
    random_bd = random.randint(1, 3)
    bd_arr = random.sample(biaodian, random_bd)
    for i in range(random_bd - 1):
        tmp = random.randint(1, len(random_word) - 1)
        random_word = random_word[:tmp - 1] + bd_arr[i] + random_word[tmp:]
    random_word = random_word[:-1] + random.choice('，。：；')
    return random_word
...
def random_x_y(bground_size, font_size):
    ...
    # print(bground_size)
    # 为防止文字溢出图片，x，y要预留宽高
    x = random.randint(0, width - font_size * 11)
...
def main(save_path, num, file):
    ...
    raw_image = create_an_image('./background/', 320, 32)
    ...
    file.write(str(num) + '.jpg ' + random_word + '\n')
    raw_image = raw_image.convert('RGB')
    raw_image.save(save_path + str(num) + '.jpg')


if __name__ == '__main__':
    with open('/home/sxs/djy/ocr/data_generator/text_recognize_data/info.txt', 'r', encoding='utf-8') as file:
        info_list = [part.strip().replace('\t', '') for part in file.readlines()]
        info_str = ''.join(info_list)

    # 图片标签
    file = open('/home/sxs/djy/ocr/data_generator/text_recognize_data/train.txt', 'w', encoding='utf-8')
    total = 500000
    for num in range(0, total):
        main('/home/sxs/djy/ocr/dataset/crnn/generator_img/', num, file)
        if num % 1000 == 0:
            print('[%d/%d]' % (num, total))
    file.close()
